# app.py
# ARAM 픽창 개인화 추천 (내 2025 전적 + CompMLP) + 스크린샷 인식(β, 좌표 잘라내기 없음)
...
def load_runes_static(lang=LANG):
    ...
    for style in data:
        ...
        if style.get("slots"):
            for r in style["slots"][0].get("runes", []):  # 슬롯0=키스톤
        ...
        try:
            ...
            for mod in self.mlp:
                if isinstance(mod, torch.nn.Linear):
                    ...
                    break
        if expect is not None and int(x.size(-1)) != expect:
            cur = int(x.size(-1))
            if not hasattr(self, "_dim_warned"):
                st.warning(f"[입력 차원 자동 보정] cur_dim={cur}, expect={expect} "
...
def predict_prob_comp(bundle, my_cid, ally_ids, enemy_ids, misc_row):
    ...
    unk_idx = len(c2i)  # 미등록 id → 마지막 인덱스
...
def suggest_runes_from_modes(cid: int, misc_modes: dict):
    ...
    psn = style_id2name.get(to_int(ps), str(ps))
    ssn = style_id2name.get(to_int(ss), str(ss))
    ksn = keystone_id2name.get(to_int(ks), str(ks))
...
# ------------------------------------------------------------------
# 🖼️ 스크린샷 업로드 → 자동 인식 (Vertex AI)  ※ 좌표 자르기 없음
# ------------------------------------------------------------------
st.markdown("---")
